...
def main():
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(test_single_tool, target): target for target in targets
        }
        ...
        for future in as_completed(futures):
            try:
                ...
                print(
                    f"{status_icon} {result['name']} - {result['status']} ({result['duration']}s)"
                )
    ...
    report_file = (
        f"logs/parallel_stress_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
