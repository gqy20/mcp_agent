...
def test_github_analyzer_with_cache():
    ...
    package_info = analyzer._extract_package_info(
        test_readme_content, deployment_methods
    )
...
def test_package_extraction_logic():
    ...
    for case in test_cases:
        ...
        package_info = analyzer._extract_package_info(
            case["content"], deployment_methods
        )
        ...
        if package_info["package_name"] == case["expected_package"]:
            ...
            print(
                f"  ❌ 包名错误，期望: {case['expected_package']}, 实际: {package_info['package_name']}"
            )
            package_ok = False
