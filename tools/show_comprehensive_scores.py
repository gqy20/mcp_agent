...
def main():
    ...
    if not supabase_url or not supabase_key:
        ...
        console.print(
            "[yellow]请设置 SUPABASE_URL 和 SUPABASE_SERVICE_ROLE_KEY 环境变量[/yellow]"
        )
        return
    ...
    for github_url in test_urls:
        ...
        if result and result.get("test_count", 0) > 0:
            ...
            tool_name = tools.get(
                github_url,
                github_url.split("/")[-1].replace("-mcp", "").replace("-", " ").title(),
            )
            if len(tool_name) > 22:
            ...
            comprehensive_score_str = (
                f"[{score_color}]{comprehensive_score}[/{score_color}]"
            )
            sustainability_str = (
                str(sustainability_score) if sustainability_score is not None else "N/A"
            )
            popularity_str = (
                str(popularity_score) if popularity_score is not None else "N/A"
            )
