...
def main():
    ...
    try:
        ...
        print(
            f"  • 综合评分: ({success_rate} × 1 + {github_score} × 2) / 3 = {total_score}"
        )

        # 检查记录是否存在
        existing = (
            client.table("mcp_repository_evaluations")
            .select("*")
            .eq("github_url", context7_url)
            .execute()
        )
        ...
        if existing.data:
            ...
            result = (
                client.table("mcp_repository_evaluations").insert(insert_data).execute()
            )
