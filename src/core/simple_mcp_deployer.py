...
class SimpleMCPDeployer:
    ...
    def __init__(self):
        ...
        print(
            f"🖥️ 平台信息: {self.platform_info['system']} ({self.platform_info['architecture']})"
        )
    ...
    def _get_runtime_info(
        self, run_command: str = None, package_name: str = None
    ) -> Dict[str, Any]:
        """获取运行时信息（npx或uvx）
    ...
    def _try_start_process(
        self, cmd, creation_flags, display_name, run_command, package_name, runtime_info
    ):
        """尝试启动进程，带--stdio回退机制"""
        try:
            # 构建进程参数（跨平台兼容）
    ...
    def deploy_package(
        ...
        try:
            ...
            if not runtime_info["available"]:
                if runtime_type == "uvx":
                    raise Exception(
                        "uvx不可用，请先安装uv: curl -LsSf https://astral.sh/uv/install.sh | sh"
                    )
                else:
            ...
            available_tools = self._initialize_mcp_protocol(
                communicator, package_name, timeout
            )
    ...
    def _initialize_mcp_protocol(
        self, communicator: SimpleMCPCommunicator, package_name: str, timeout: int
    ) -> List[Dict[str, Any]]:
        """初始化MCP协议并获取工具列表"""
        ...
        if not init_result["success"]:
            ...
            init_result = communicator.send_request(
                simple_init_request, timeout=timeout
            )
            if not init_result["success"]:
