...
class PathsConfig:
    ...
    mcp_database_dir: Path = field(
        default_factory=lambda: Path(__file__).parent.parent.parent.parent
        / "data"
        / "mcp_database",
    )
    mcp_csv_path: Path = field(
        default_factory=lambda: Path(__file__).parent.parent.parent.parent
        / "data"
        / "mcp_database"
        / "mcp.csv",
...
class TimeoutsConfig:
    ...
    deployment_timeout: int = field(
        default_factory=lambda: int(os.getenv("DEPLOYMENT_TIMEOUT", "120")),
    )
    install_timeout: int = field(
        default_factory=lambda: int(os.getenv("INSTALL_TIMEOUT", "300")),
    )
    ...
    communication_timeout: int = field(
...
class TestingConfig:
    ...
    # 默认启用功能
    enable_smart_testing: bool = field(
        default_factory=lambda: os.getenv("ENABLE_SMART_TESTING", "true").lower()
        == "true",
    )
    enable_database_export: bool = field(
        default_factory=lambda: os.getenv("ENABLE_DATABASE_EXPORT", "true").lower()
        == "true",
    )
    enable_evaluation: bool = field(
        default_factory=lambda: os.getenv("ENABLE_EVALUATION", "true").lower()
        == "true",
    )

    # 测试限制
    max_test_tools: int = field(
        default_factory=lambda: int(os.getenv("MAX_TEST_TOOLS", "100")),
    )
    concurrent_tests: int = field(
        default_factory=lambda: int(os.getenv("CONCURRENT_TESTS", "3")),
    )

    # LobeHub评分权重
    final_score_weights: dict[str, float] = field(
        default_factory=lambda: {"success_rate": 1.0, "evaluator_score": 2.0},
    )
...
class SystemConfig:
    ...
    # 支持的部署方法
    supported_deployment_methods: list[str] = field(
        default_factory=lambda: ["npx", "npm", "pip", "uvx", "cargo", "docker"],
    )
...
class AppConfig:
    ...
    def get_summary(self) -> dict[str, str | bool | int]:
        """获取配置摘要."""
        return {
