...
class TestMCPTesterHTTPSupport:
    ...
    @pytest.mark.asyncio
    async def test_test_http_client_success(self, mcp_tester):
        """测试：HTTP 客户端成功测试"""
        # 创建模拟的 HTTP 客户端
    ...
    def test_construct_test_args_with_different_schemas(self, mcp_tester):
        """测试：使用不同输入模式构造测试参数"""
        test_cases = [
            {
                'tool': {
        ...
        for i, test_case in enumerate(test_cases):
            ...
            for key in test_case['expected_keys']:
                assert key in args, f"Test case {i}: Expected key '{key}' not found in args"
    ...
    async def test_http_client_timeout_handling(self, mcp_tester):
        ...
        # 模拟超时
        mock_client.call_tool.side_effect = asyncio.TimeoutError("Operation timed out")
    ...
    async def test_http_client_multiple_tools_testing(self, mcp_tester):
        ...
        def call_tool_side_effect(name, args):
            if name == 'tool1':
                return {'success': True, 'result': {'content': [{'type': 'text', 'text': 'Tool1 result'}]}}
            elif name == 'tool2':
                return {'success': True, 'result': {'content': [{'type': 'text', 'text': 'Tool2 result'}]}}
            else:
